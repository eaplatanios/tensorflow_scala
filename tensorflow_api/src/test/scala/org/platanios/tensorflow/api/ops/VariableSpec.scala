...
class VariableSpec extends FlatSpec with Matchers {
  "Variable creation" must "work" in {
    ...
    val variable = createWith(graph = graph) {
      ...
      Variable(initializer, shape = Shape(1, 2), dataType = TFInt64)
    }
    assert(variable.dataType === TFInt64)
    ...
    val expectedResult = Tensor(TFInt64, Tensor(2, 3))
  ...
  "Variable assignment" must "work" in {
    ...
    val (variable, variableAssignment) = createWith(graph = graph) {
      val a = constant(Tensor(Tensor(5, 7)), TFInt64, name = "A")
      val initializer = Variable.ConstantInitializer(Tensor(Tensor(2, 3)))
      val variable = Variable(initializer, shape = Shape(1, 2), dataType = TFInt64)
    ...
    val expectedResult = Tensor(TFInt64, Tensor(5, 7))
