...
class DataTypeSpec extends FlatSpec with Matchers {
  ...
  "'DataType.equals'" must "always work correctly" in {
    assert(DataType.Float16 === DataType.Float16)
    assert(DataType.Float16 !== DataType.Float32)
  ...
  "'DataType.cast'" must "work correctly when provided values of supported types" in {
    ...
    // TODO: Add complex data types checks.
    ...
    assert(DataType.UInt16.cast(-UByte(2)) === UShort(65534)) // TODO: Should this throw an error?
    ...
    assert(DataType.QUInt16.cast(-UByte(2)) === UShort(65534)) // TODO: Should this throw an error?
    ...
    assert(DataType.Boolean.cast(false) === false)
    assert(DataType.Boolean.cast(true) === true)
  ...
  "'DataType.putElementInBuffer'" must "work correctly when provided values of supported types" in {
    ...
    assert(buffer.get(0) === 0x00)
    assert(buffer.get(1) === 0x00)
    assert(buffer.get(2) === 0x00)
    assert(buffer.get(3) === 0x01)
    assert(buffer.get(4) === 0x00)
    assert(buffer.get(5) === 0x00)
    assert(buffer.get(6) === 0x00)
    assert(buffer.get(7) === 0x10)
    assert(buffer.get(8) === 0x00)
    assert(buffer.get(9) === 0x00)
    assert(buffer.get(10) === 0x01)
    assert(buffer.get(11) === 0x01)
  ...
  "'DataType.getElementFromBuffer'" must "match the behavior of 'DataType.putElementInBuffer'" in {
    ...
    DataType.Boolean.putElementInBuffer(buffer, 44, true)
    DataType.Boolean.putElementInBuffer(buffer, 45, false)
    ...
    // TODO: What data type should we actually use for this?
    ...
    assert(DataType.Boolean.getElementFromBuffer(buffer, 44) === true)
    assert(DataType.Boolean.getElementFromBuffer(buffer, 45) === false)
  }
}
