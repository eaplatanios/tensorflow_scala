...
/** Adds functionality for generating JNI header and implementation files for executing TensorFlow eager tensor ops.
  ...
  */
object TensorOpsGeneration extends AutoPlugin {
  object autoImport {
    ...
    val ops: TaskKey[Map[String, Seq[String]]] = taskKey[Map[String, Seq[String]]](
      "Grouped ops for which to generate bindings.")

    val scalaPackage: TaskKey[String] = taskKey[String](
      "Grouped ops for which to generate bindings.")
  }
  ...
  lazy val settings: Seq[Setting[_]] = Seq(
    generateTensorOps := generateFiles(
    ...
    clean in generateTensorOps := {
      streams.value.log.debug("Cleaning generated TensorFlow tensor op files.")
    ...
    clean := {
      (clean in generateTensorOps).value
      clean.value
    }
  )
  ...
  private[this] def generateFiles(path: Path, ops: Map[String, Seq[String]], scalaPackage: String): Unit = {
  ...
  private[this] def code(
    ...
    val jniObjectName = s"$scalaPackage.$objectName".replace(".", "_") + objectName
  ...
  private[this] case class OpGenerator(opDef: OpDef) {
    ...
    def jniNative(className: String = "org_platanios_tensorflow_jni_Tensor"): (String, String, String) = {
      ...
      val scalaFunction =
        s"""  @native def $name($scalaArgs): $scalaReturnType""".stripMargin
    ...
    private[this] val numAttrs   = opDef.getAttrCount
    private[this] val numOutputs = opDef.getOutputArgCount
    ...
    private[this] val outputs = mutable.ListBuffer.empty[String]
    ...
    private[this] def processArgs(): Unit = {
      ...
      val argsWithoutDefaults = mutable.ListBuffer.empty[(String, String)]
      ...
      val argsWithDefaults = mutable.ListBuffer.empty[((String, String), String)]
      ...
      opDef.getInputArgList.asScala.zipWithIndex.foreach { case (arg, index) =>
        ...
        argsWithoutDefaults.append((arg.getName, scalaType))
      }
        ...
        if (_isSupported) {
          if (attr.hasDefaultValue) {
            ...
            if (scalaValue.isDefined)
              argsWithDefaults.append(((attr.getName, scalaType.get), scalaValue.get))
            else
              setUnsupported()
          } else {
            argsWithoutDefaults.append((attr.getName, scalaType.get))
          }
        }
      }
      ...
      if (_isSupported) {
        ...
        attrs.appendAll(argsWithoutDefaults.drop(numInputs))
        attrs.appendAll(argsWithDefaults.map(_._1))
              ...
              if (!attrExpressions.contains(attrName)) {
                ...
                implementationBuilder.append(
                  s"""
                     ...
                     |      throw_exception(env, jvm_illegal_argument_exception, error_msg.str());
                     |  }""".stripMargin)
              }
            })
          case "type" =>
                ...
                case "Long" =>
                  if (!attrExpressions.contains(attrName)) {
                    ...
                    implementationBuilder.append(
                      s"""
                         |
                         |  const TF_DataType $attrValueName = TFE_TensorHandleDataType($inputName);""".stripMargin)
                  } else {
                    val attrValueName = attrExpressions(attrName)
                    implementationBuilder.append(
                      s"""
                         |
                         |  const TF_DataType ${inputName}_attr_$attrName = TFE_TensorHandleDataType($inputName);
                         |  if ($attrValueName != ${inputName}_attr_$attrName) {
                         |      std::stringstream error_msg;
                         |      error_msg
    ...
    private[this] def addInputs(implementationBuilder: mutable.StringBuilder, nullReturnValue: String): Unit = {
      params.drop(numInputs).foreach(param => {
    ...
    private[this] def addAttributes(
      ...
      opDef.getAttrList.asScala.foreach(attr => {
        val attrName = attr.getName
        val attrType = attr.getType
        val value = attrExpressions(attrName)
        attrType match {
          case "string" =>
            implementationBuilder.append(
              s"""
  ...
  private[this] object OpGenerator {
