...
abstract class Dataset[T, O, D, S] private[io](val name: String = "Dataset")(implicit ev: Data.Aux[T, O, D, S]) {
  ...
  /** $OpDocDatasetMap
    ...
    * @return Created dataset.
    */
  def map[RT, RO, RD, RS](function: (O) => RO, numParallelCalls: Int = 1, bufferSize: Long = 1)(implicit
  ...
  /** $OpDocDatasetFlatMap
    ...
    * @return Created dataset.
    */
  def flatMap[RT, RO, RD, RS](function: (O) => Dataset[RT, RO, RD, RS])(implicit
      ev: Data.Aux[T, O, D, S],
      evR: Data.Aux[RT, RO, RD, RS],
      evFunctionInput: Function.ArgType[O]
  ): Dataset[RT, RO, RD, RS] = {
...
object Dataset {
  ...
  /** Creates a [[Dataset]] that splits a sparse tensor into its rows.
    ...
    * @return Created dataset.
    */
  private[api] def fromSparseSlices(tensor: SparseTensor, name: String = "SparseTensorSlicesDataset"):
  Dataset[SparseTensor, SparseOutput, (DataType, DataType, DataType), (Shape, Shape, Shape)] = {
  ...
  /** Creates a [[Dataset]] that splits a sparse tensor into its rows.
    ...
    * @return Created dataset.
    */
  private[api] def fromSparseOutputSlices(tensor: SparseOutput, name: String = "SparseOutputSlicesDataset"):
  Dataset[SparseTensor, SparseOutput, (DataType, DataType, DataType), (Shape, Shape, Shape)] = {
  ...
  def map[T, O, D, S, RT, RO, RD, RS](
      dataset: Dataset[T, O, D, S], function: (O) => RO, numParallelCalls: Int = 1, bufferSize: Long = 1,
  ...
  def flatMap[T, O, D, S, RT, RO, RD, RS](
      dataset: Dataset[T, O, D, S], function: (O) => Dataset[RT, RO, RD, RS], name: String = "FlatMapDataset")(implicit
      ev: Data.Aux[T, O, D, S],
      evR: Data.Aux[RT, RO, RD, RS],
      evFunctionInput: Function.ArgType[O]
  ): Dataset[RT, RO, RD, RS] = {
...
// TODO: !!! [DATASETS] Tensor and output datasets should be one class (the outputs one). However, datasets should be made more functional so they can be created lazily.
...
/** [[Dataset]] with a single element.
  ...
  * @param  name Name for this dataset.
  */
private[io] case class OutputDataset[T, O, D, S] private[io](
    data: O, override val name: String = "OutputDataset")(implicit
    ev: Data.Aux[T, O, D, S]
) extends Dataset[T, O, D, S](name) {
  override def createHandle(): Output = {
...
/** [[Dataset]] with slices from the nested structure of [[Tensor]]s (i.e., a [[Data]]-supported type). The slices are
  ...
  * @param  name Name for this dataset.
  */
private[io] case class TensorSlicesDataset[T, O, D, S] private[io](
    data: T, override val name: String = "TensorSlicesDataset")(implicit
    ev: Data.Aux[T, O, D, S]
) extends Dataset[T, O, D, S](name) {
  override def createHandle(): Output = {
    val flattenedOutputs = ev.flattenedOutputsFromT(data)
...
/** [[Dataset]] with slices from the nested structure of [[Output]]s (i.e., a [[Data]]-supported type). The slices are
  ...
  * @param  name Name for this dataset.
  */
private[io] case class OutputSlicesDataset[T, O, D, S] private[io](
    data: O, override val name: String = "OutputSlicesDataset")(implicit
    ev: Data.Aux[T, O, D, S]
) extends Dataset[T, O, D, S](name) {
  override def createHandle(): Output = {
    val flattenedOutputs = ev.flattenedOutputsFromO(data)
...
/** [[Dataset]] that splits a sparse tensor into its rows.
  ...
  * @param  name   Name for this dataset.
  */
private[io] case class SparseTensorSlicesDataset private[io](
...
/** [[Dataset]] that splits a sparse tensor into its rows.
  ...
  * @param  name   Name for this dataset.
  */
private[io] case class SparseOutputSlicesDataset private[io](
    tensor: SparseOutput, override val name: String = "SparseOutputSliceDataset")
    extends Dataset[SparseTensor, SparseOutput, (DataType, DataType, DataType), (Shape, Shape, Shape)](name) {
  /** Creates a `RESOURCE` scalar tensor representing this dataset. This function adds ops to the current graph, that
    * create the dataset resource. */
  override def createHandle(): Output = {
