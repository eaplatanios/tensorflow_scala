...
private[api] trait Summary {
  ...
  def mergeSummaries(
    ...
    Op.createWithNameScope(cleanedName, summaries.map(_.op)) {
      val merged = Summary.mergeSummariesOp(summaries.toSeq, cleanedName)
  ...
  def mergeAllSummaries(
      key: Graph.Key[Output] = Graph.Keys.SUMMARIES, name: String = "AllSummariesMerge"): Option[Output] = {
    val summaries = Op.currentGraph.getCollection(key)
    if (summaries.isEmpty)
      None
    else
      Some(mergeSummaries(summaries, name = name))
  }
}
...
//  }
  ...
  /** $OpDocSummaryMergeSummaries
    ...
    *         resulting from the merge.
    */
  private[Summary] def mergeSummariesOp(inputs: Seq[Output], name: String = "MergeSummaries"): Output = {
  ...
  private[Summary] def writeScalarSummary(
      writerHandle: Output, globalStep: Output, value: Output, tag: Output, name: String = "WriteScalarSummary"): Op = {
  ...
  /** $OpDocSummaryWriteImageSummary
    ...
    * @param  writerHandle    Handle to a summary writer resource.
    * @param  globalStep      [[INT64]] tensor containing the global step to write the summary for.
    * @param  tensor          Four-dimensional tensor with shape `[batchSize, height, width, channels]` where `channels`
    *                         is 1, 3, or 4.
  ...
  /** $OpDocSummaryWriteAudioSummary
    ...
    * @param  writerHandle    Handle to a summary writer resource.
    * @param  globalStep      [[INT64]] tensor containing the global step to write the summary for.
    * @param  tensor          Three-dimensional tensor with shape `[batchSize, frames, channels]` or two-dimensional
    *                         tensor with shape `[batchSize, frames]`.
  ...
  /** @define OpDocSummaryMergeSummaries
    ...
    *
    * @define OpDocSummaryWriteSummary
    *   The `writeSummary` op writes a `Summary` protocol buffer containing a tensor, to the provided summary writer.
    *
    ...
    *
    *   This op thrown an `InvalidArgumentException` is any of the provided values is not finite.
    *
