...
/** Represents a gradients packer that helps facilitate faster communication between devices.
  *
  * TODO: [DISTRIBUTE] This can only be used with dense gradients at this point.
  *
...
trait Packer[P] {
  ...
  def pack(groupedGradientsAndVariables: Seq[Seq[(Output, Variable)]]): (Seq[Seq[Output]], Option[P])
  ...
  /** Reverses the packing performed by `pack`, on the provided packed gradients.
    ...
    */
  @throws[InvalidArgumentException]
  def unpack(packedGradients: Seq[Seq[(Output, Variable)]], packInformation: Option[P]): Seq[Seq[(Output, Variable)]]
}
