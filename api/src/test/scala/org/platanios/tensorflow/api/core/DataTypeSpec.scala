...
class DataTypeSpec extends FlatSpec with Matchers {
  "'tf.dataTypeOf'" must "work correctly when valid values are provided" in {
    assert(tf.dataTypeOf(1.0f) === tf.FLOAT32)
    assert(tf.dataTypeOf(1.0) === tf.FLOAT64)
    assert(tf.dataTypeOf(1.asInstanceOf[Byte]) === tf.INT8)
    assert(tf.dataTypeOf(1.asInstanceOf[Short]) === tf.INT16)
    assert(tf.dataTypeOf(1) === tf.INT32)
    assert(tf.dataTypeOf(1L) === tf.INT64)
    assert(tf.dataTypeOf(UByte(1)) === tf.UINT8)
    assert(tf.dataTypeOf(UShort(1)) === tf.UINT16)
    // assert(tf.dataTypeOf('a') === tf.UINT16)
    assert(tf.dataTypeOf(true) === tf.BOOLEAN)
    // assert(tf.dataTypeOf("foo") === tf.STRING)
    // assert(tf.dataTypeOf(Tensor(1, 2)) === tf.INT32)
  }

  it must "not compile when invalid values are provided" in {
    assertDoesNotCompile("val d: tf.DataType = tf.dataTypeOf(Array(1))")
    assertDoesNotCompile("val d: tf.DataType = tf.dataTypeOf(1.asInstanceOf[Any])")
  }

  "'tf.dataType'" must "work correctly when valid C values are provided" in {
    assert(tf.dataType(1) === tf.FLOAT32)
    assert(tf.dataType(2) === tf.FLOAT64)
    assert(tf.dataType(3) === tf.INT32)
    assert(tf.dataType(4) === tf.UINT8)
    assert(tf.dataType(5) === tf.INT16)
    assert(tf.dataType(6) === tf.INT8)
    assert(tf.dataType(7) === tf.STRING)
    assert(tf.dataType(8) === tf.COMPLEX64)
    assert(tf.dataType(9) === tf.INT64)
    assert(tf.dataType(10) === tf.BOOLEAN)
    assert(tf.dataType(11) === tf.QINT8)
    assert(tf.dataType(12) === tf.QUINT8)
    assert(tf.dataType(13) === tf.QINT32)
    assert(tf.dataType(14) === tf.BFLOAT16)
    assert(tf.dataType(15) === tf.QINT16)
    assert(tf.dataType(16) === tf.QUINT16)
    assert(tf.dataType(17) === tf.UINT16)
    assert(tf.dataType(18) === tf.COMPLEX128)
    assert(tf.dataType(19) === tf.FLOAT16)
    assert(tf.dataType(20) === tf.RESOURCE)
  }
  ...
  "'tf.dataType'" must "work correctly when valid data type names are provided" in {
    assert(tf.dataType("FLOAT32") === tf.FLOAT32)
    assert(tf.dataType("FLOAT64") === tf.FLOAT64)
    assert(tf.dataType("INT32") === tf.INT32)
    assert(tf.dataType("UINT8") === tf.UINT8)
    assert(tf.dataType("INT16") === tf.INT16)
    assert(tf.dataType("INT8") === tf.INT8)
    assert(tf.dataType("STRING") === tf.STRING)
    assert(tf.dataType("COMPLEX64") === tf.COMPLEX64)
    assert(tf.dataType("INT64") === tf.INT64)
    assert(tf.dataType("BOOLEAN") === tf.BOOLEAN)
    assert(tf.dataType("QINT8") === tf.QINT8)
    assert(tf.dataType("QUINT8") === tf.QUINT8)
    assert(tf.dataType("QINT32") === tf.QINT32)
    assert(tf.dataType("BFLOAT16") === tf.BFLOAT16)
    assert(tf.dataType("QINT16") === tf.QINT16)
    assert(tf.dataType("QUINT16") === tf.QUINT16)
    assert(tf.dataType("UINT16") === tf.UINT16)
    assert(tf.dataType("COMPLEX128") === tf.COMPLEX128)
    assert(tf.dataType("FLOAT16") === tf.FLOAT16)
    assert(tf.dataType("RESOURCE") === tf.RESOURCE)
  }

  it must "throw an 'IllegalArgumentException' when invalid data type names are provided" in {
    assertThrows[IllegalArgumentException](tf.dataType("foo"))
    assertThrows[IllegalArgumentException](tf.dataType("bar"))
    assertThrows[IllegalArgumentException](tf.dataType(""))
    assertThrows[IllegalArgumentException](tf.dataType(null))
  }
  ...
  "'DataType.isUnsigned'" must "always work correctly" in {
   ...
   "'DataType.real'" must "always work correctly" in {
     assert(tf.BOOLEAN.real === tf.BOOLEAN)
     assert(tf.STRING.real === tf.STRING)
  ...
  it must "not compile when invalid values are provided" in {
    assertDoesNotCompile("tf.FLOAT32.cast(Array(1))")
    assertDoesNotCompile("tf.FLOAT32.cast(1.asInstanceOf[Any])")
  }
