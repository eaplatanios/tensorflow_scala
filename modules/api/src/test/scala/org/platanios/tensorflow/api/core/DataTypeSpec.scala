...
class DataTypeSpec extends FlatSpec with Matchers {
  "'tf.dataTypeOf'" must "work correctly when valid values are provided" in {
    assert(tf.dataTypeOf(1.0f) === FLOAT32)
    assert(tf.dataTypeOf(1.0) === FLOAT64)
    assert(tf.dataTypeOf(1.asInstanceOf[Byte]) === INT8)
    assert(tf.dataTypeOf(1.asInstanceOf[Short]) === INT16)
    assert(tf.dataTypeOf(1) === INT32)
    assert(tf.dataTypeOf(1L) === INT64)
    assert(tf.dataTypeOf(UByte(1)) === UINT8)
    assert(tf.dataTypeOf(UShort(1)) === UINT16)
    // assert(tf.dataTypeOf('a') === UINT16)
    assert(tf.dataTypeOf(true) === BOOLEAN)
    // assert(tf.dataTypeOf("foo") === STRING)
    // assert(tf.dataTypeOf(Tensor(1, 2)) === INT32)
  }

  it must "not compile when invalid values are provided" in {
    assertDoesNotCompile("val d: DataType = tf.dataTypeOf(Array(1))")
    assertDoesNotCompile("val d: DataType = tf.dataTypeOf(1.asInstanceOf[Any])")
  }

  "'tf.dataType'" must "work correctly when valid C values are provided" in {
    assert(tf.dataType(1) === FLOAT32)
    assert(tf.dataType(2) === FLOAT64)
    assert(tf.dataType(3) === INT32)
    assert(tf.dataType(4) === UINT8)
    assert(tf.dataType(5) === INT16)
    assert(tf.dataType(6) === INT8)
    assert(tf.dataType(7) === STRING)
    assert(tf.dataType(8) === COMPLEX64)
    assert(tf.dataType(9) === INT64)
    assert(tf.dataType(10) === BOOLEAN)
    assert(tf.dataType(11) === QINT8)
    assert(tf.dataType(12) === QUINT8)
    assert(tf.dataType(13) === QINT32)
    assert(tf.dataType(14) === BFLOAT16)
    assert(tf.dataType(15) === QINT16)
    assert(tf.dataType(16) === QUINT16)
    assert(tf.dataType(17) === UINT16)
  ...
  "'tf.dataType'" must "work correctly when valid data type names are provided" in {
    assert(tf.dataType("FLOAT32") === FLOAT32)
    assert(tf.dataType("FLOAT64") === FLOAT64)
    assert(tf.dataType("INT32") === INT32)
    assert(tf.dataType("UINT8") === UINT8)
    assert(tf.dataType("INT16") === INT16)
    assert(tf.dataType("INT8") === INT8)
    assert(tf.dataType("STRING") === STRING)
    assert(tf.dataType("COMPLEX64") === COMPLEX64)
    assert(tf.dataType("INT64") === INT64)
    assert(tf.dataType("BOOLEAN") === BOOLEAN)
    assert(tf.dataType("QINT8") === QINT8)
    assert(tf.dataType("QUINT8") === QUINT8)
    assert(tf.dataType("QINT32") === QINT32)
    assert(tf.dataType("BFLOAT16") === BFLOAT16)
    assert(tf.dataType("QINT16") === QINT16)
    assert(tf.dataType("QUINT16") === QUINT16)
    assert(tf.dataType("UINT16") === UINT16)
  ...
  "'DataType.isUnsigned'" must "always work correctly" in {
   ...
   "'DataType.real'" must "always work correctly" in {
     assert(BOOLEAN.real === BOOLEAN)
     assert(STRING.real === STRING)
