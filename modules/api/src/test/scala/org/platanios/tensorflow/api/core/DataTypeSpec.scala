...
class DataTypeSpec extends FlatSpec with Matchers {
  ...
  "'DataType.fromName'" must "work correctly when valid data type names are provided" in {
    ...
    assert(DataType.fromName("VARIANT") == VARIANT)
  }
  ...
  it must "not compile when invalid values are provided" in {
    assertDoesNotCompile("FLOAT32.cast(Array(1))")
    assertDoesNotCompile("FLOAT32.cast(1.asInstanceOf[Any])")
  }
  ...
  "'DataType.putElementInBuffer'" must "work correctly when provided values of supported types" in {
    ...
    DataType.putElementInBuffer(buffer, 0, INT32, 1)
    DataType.putElementInBuffer(buffer, 4, INT32, 16)
    DataType.putElementInBuffer(buffer, 8, INT32, 257)
  ...
  it must "throw an 'IndexOutOfBoundsException' exception when writing beyond the buffer boundaries" in {
    ...
    DataType.putElementInBuffer(buffer, 0, INT32, 1)
    DataType.putElementInBuffer(buffer, 4, INT32, 16)
    assertThrows[IndexOutOfBoundsException](DataType.putElementInBuffer(buffer, 8, INT32, 257))
  }
  ...
  "'DataType.getElementFromBuffer'" must "match the behavior of 'DataType.putElementInBuffer'" in {
    ...
    // DataType.putElementInBuffer(buffer, 0, FLOAT16, 2f)
    DataType.putElementInBuffer(buffer, 2, FLOAT32, -4.23f)
    DataType.putElementInBuffer(buffer, 6, FLOAT64, 3.45)
    // DataType.putElementInBuffer(buffer, 14, BFLOAT16, -1.23f)
    DataType.putElementInBuffer(buffer, 16, INT8, 4.toByte)
    DataType.putElementInBuffer(buffer, 17, INT16, (-2).toShort)
    DataType.putElementInBuffer(buffer, 19, INT32, 54)
    DataType.putElementInBuffer(buffer, 23, INT64, -3416L)
    //    DataType.putElementInBuffer(buffer, 31, UINT8, UByte(34))
    //    DataType.putElementInBuffer(buffer, 32, UINT16, UShort(657))
    //    DataType.putElementInBuffer(buffer, 34, QINT8, (-4).toByte)
    //    DataType.putElementInBuffer(buffer, 35, QINT16, 32.toShort)
    //    DataType.putElementInBuffer(buffer, 37, QINT32, -548979)
    //    DataType.putElementInBuffer(buffer, 41, QUINT8, UByte(254))
    //    DataType.putElementInBuffer(buffer, 42, QUINT16, UShort(765))
    DataType.putElementInBuffer(buffer, 44, BOOLEAN, true)
    DataType.putElementInBuffer(buffer, 45, BOOLEAN, false)
    // TODO: Add checks for the string data type.
    // assert(DataType.getElementFromBuffer(buffer, 0, FLOAT16) === 2f)
    assert(DataType.getElementFromBuffer(buffer, 2, FLOAT32) === -4.23f)
    assert(DataType.getElementFromBuffer(buffer, 6, FLOAT64) === 3.45)
    // assert(DataType.getElementFromBuffer(buffer, 14, BFLOAT16) === -1.23f)
    assert(DataType.getElementFromBuffer(buffer, 16, INT8) === 4.toByte)
    assert(DataType.getElementFromBuffer(buffer, 17, INT16) === (-2).toShort)
    assert(DataType.getElementFromBuffer(buffer, 19, INT32) === 54)
    assert(DataType.getElementFromBuffer(buffer, 23, INT64) === -3416L)
    //    assert(DataType.getElementFromBuffer(buffer, 31, UINT8) === UByte(34))
    //    assert(DataType.getElementFromBuffer(buffer, 32, UINT16) === UShort(657))
    //    assert(DataType.getElementFromBuffer(buffer, 34, QINT8) === (-4).toByte)
    //    assert(DataType.getElementFromBuffer(buffer, 35, QINT16) === 32.toShort)
    //    assert(DataType.getElementFromBuffer(buffer, 37, QINT32) === -548979)
    //    assert(DataType.getElementFromBuffer(buffer, 41, QUINT8) === UByte(254))
    //    assert(DataType.getElementFromBuffer(buffer, 42, QUINT16) === UShort(765))
    assert(DataType.getElementFromBuffer(buffer, 44, BOOLEAN) === true)
    assert(DataType.getElementFromBuffer(buffer, 45, BOOLEAN) === false)
  }
}
