...
object CRC32C {
  /** Returns the CRC32C of the provided string.
    *
    * @param  value String for which to compute the CRC32C.
    * @return CRC32C for `value`.
    */
  def value(value: String): Int = extend(0, value)

  /** Returns the CRC32C of `a` concatenated with `value` where `initCrc` is the CRC32C of some string `a`. This method
    * is often used to maintain the CRC32C of a stream of data.
    *
    * @param  initCrc CRC32C of some string.
    * @param  value   String to use in order to extend `initCrc`.
    * @return Extended CRC32C.
    */
  def extend(initCrc: Int, value: String): Int = {
    val bytes = value.getBytes()
    extend(initCrc, bytes, bytes.length)
  }

  /** Returns the CRC32C of `data[0, size - 1])`.
    *
    * @param  bytes Bytes to use in order to compute the CRC32C.
    * @param  size  Number of bytes to use from `bytes`. If `-1` (the default value), then all bytes are used.
    * @return CRC32C for `bytes`.
    */
  def value(bytes: Array[Byte], size: Int = -1): Int = extend(0, bytes, size)

  /** Returns the CRC32C of `a` concatenated with `data[0, size - 1])` where `initCrc` is the CRC32C of some string `a`.
    * This method is often used to maintain the CRC32C of a stream of data.
    *
    * @param  initCrc CRC32C of some string.
    * @param  bytes   Bytes to use in order to extend `initCrc`.
    * @param  size    Number of bytes to use from `bytes`. If `-1` (the default value), then all bytes are used.
    * @return Extended CRC32C.
    */
  def extend(initCrc: Int, bytes: Array[Byte], size: Int = -1): Int = {
    val n = if (size == -1) bytes.length else size
    var l = initCrc ^ 0xffffffffL
    // Point x at first 4-byte aligned byte in the bytes array. This might be just past the end of the array.
    var i = 0
    // Process bytes 4 at a time.
    // TODO: !!! [UTILITIES] There is something wrong with this.
    // while (i < n - 4) {
    //   l = step4(l, bytes, i) & 0xffffffffL
    //   i += 4
    // }
    // Process the last few bytes.
    while (i < n) {
      l = step1(l, bytes, i) & 0xffffffffL
      i += 1
    }
    (l ^ 0xffffffffL).toInt
  }
  ...
  /** Returns a masked representation of `crc`.
    *
    * Motivation: It is problematic to compute the CRC of a byte array that contains embedded CRCs. Therefore, we
    * recommend that CRCs stored somewhere (e.g., in files) should be masked before being stored.
    *
    * @param  crc CRC to mask.
    * @return Masked representation of `crc`.
    */
  def mask(crc: Int): Int = {
    // Rotate right by 15 bits and add a constant.
    val l = crc & 0xffffffffL
    (((l >> 15) | (l << 17)) + maskDelta).toInt
  }

  /** Returns the CRC whose masked representation is `maskedCrc`.
    *
    * @param  maskedCrc Masked representation of a CRC.
    * @return CRC whose masked representation is `maskedCrc`.
    */
  def unmask(maskedCrc: Int): Int = {
    val l = maskedCrc & 0xffffffffL
    val rot = l - maskDelta
    ((rot >> 17) | (rot << 15)).toInt
  }

  @inline private[this] def step1(l: Long, bytes: Array[Byte], offset: Int): Long = {
    val c = l ^ bytes(offset)
    table0((c & 0xff).toInt) ^ (l >> 8)
  }

  @inline private[this] def step4(l: Long, bytes: Array[Byte], offset: Int): Long = {
    val c = l ^ Coding.decodeFixedInt32(bytes, offset)
    table3((c & 0xff).toInt) ^
        table2(((c >> 8) & 0xff).toInt) ^
        table1(((c >> 16) & 0xff).toInt) ^
        table0(((c >> 24) & 0xff).toInt)
  }

  private[this] val maskDelta: Long = 0xa282ead8L

  private[this] val table0: Array[Int] = Array(
  ...
  private[this] val table1: Array[Int] = Array(
  ...
  private[this] val table2: Array[Int] = Array(
  ...
  private[this] val table3: Array[Int] = Array(
